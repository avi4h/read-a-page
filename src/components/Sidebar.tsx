...
export const Sidebar: React.FC<SidebarProps> = () => {
    ...
    return (
        <>
            <div
                className={`fixed inset-0 bg-black/30 dark:bg-black/50 z-50 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                onClick={onClose}
                aria-hidden="true"
            ></div>
            <div
                className={`fixed top-0 right-0 h-full w-80 bg-white dark:bg-slate-900 z-[60] shadow-xl transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
            >
                <div className="flex flex-col h-full">
                    ...
                    <nav className="flex-grow p-4 space-y-2">
                        <SidebarLink icon={<LogoIcon className="w-6 h-6" />} onClick={() => handleNavigate('reading')} active={activeView === 'reading'}>Read a Page</SidebarLink>
